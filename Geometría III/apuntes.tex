...
\begin{center}
  ...
  \begin{tikzpicture}[scale=2]
    ...
    \end{pgfonlayer}{1}
    \begin{pgfonlayer}{3}
      \node [fill=500,inner sep=1pt,label=below:$M_{AB}$, scale=1.5, circle] (MAB) at
      ($ (A)!.5!(B)$) {};
    \end{pgfonlayer}{3}
    \coordinate (C1) at ($ (MAB)!1.2!(C)$);
    \coordinate (C2) at ($ (MAB)!-.3!(C)$);
    \draw [dashed, 300, name path=MC](C1) -- (C2);
    \begin{pgfonlayer}{3}
      \node [fill=500,inner sep=1pt,label=left:$M_{AC}$, scale=1.5, circle] (MAC) at ($
      (A)!.5!(C)$) {};
    \end{pgfonlayer}{3}
    \coordinate (B1) at ($ (MAC)!1.2!(B) $);
    \coordinate (B2) at ($ (MAC)!-.3!(B)$);
    
    \draw [dashed, 300, name path=MB] (B1) -- (B2);
