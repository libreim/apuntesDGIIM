...
\begin{ndef}
	$ABCD$ es un cuadrilátero que está formado por 4 puntos consecutivos no alineados 3 a 3. Se llaman lados a los segmentos que unen los vertices consecutivos, y diagonales a los segmentos que unen $AvC$ y $BvD$.
\end{ndef}

\begin{ndef}[Lado]
	Un lado es una recta que une dos vértices consecutivos. En concreto, los cuadriláteros tienen 4 lados: $AvB$, $BvC$, $CvD$
	y $DvA$.
\end{ndef}

\begin{ndef}[Diagonal]
	Una diagonal es una recta que une dos vertices no consecutivos. En concreto, los cuadriláteros tienen 2 diagonales: $AvC$ y $BvD$. 
...
\begin{ndef}[Paralelogramo]
	...
	paralelogramo entonces $AvB$ es paralelo a $CvD$ y $BvC$ es paralelo a $DvA$. Además los paralelogramos cumplen que:
	- Sea $V_{1} = AvB$, $V_{2} = BvC$, $V_{3} = CvD$, $V_{4} = DvA$ entonces:
	\begin{nlist}
