...
\begin{nprop}\hfill
\begin{nlist}
\item La ecuación \ref{lin:completa} es estable si, y solo si la función $a(t)$ tiene una primitiva acotada superiormente en $[t_0,+\infty)$.
\item La ecuación \ref{lin:completa} es a.e. si, y solo si $\lim_{t\to +\infty} \int^t_{t_0} a(s) ds = -\infty$.
\end{nlist}
\end{nprop}
Ejemplo 1:
- Inestable
...
\begin{notacion}[Multiplicidad (aritmética)]
...
Consideramos la EDO lineal homogénea y autónoma \ref{lin:homogenea}. El principal indicador para determinar la estabilidad de esta es el máximo de las partes reales de los valores propios de A:

\[ \mu(A)=\max\{\mathrm{Re}(\lambda) : \lambda\in\sigma(A)\}. \]

\begin{nth} Las siguientes afirmaciones son verdaderas:
\begin{nlist}
