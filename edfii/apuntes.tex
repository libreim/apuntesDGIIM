...
\begin{nprop}
  ...
  \begin{nlist}
  \item Todas las soluciones de \ref{lin:PVI} son estables, 
  \item existe una solución de \ref{lin:PVI} que es estable,
  \item la solución trivial ($\varphi = 0$) de \ref{lin:PVIH} es estable,
  \item todas las soluciones de \ref{lin:PVIH} son acotadas en $[t_0, +\infty)$,
  \item la m.f.p. en $t_0$ de \ref{lin:PVIH} es acotada en $[t_0, +\infty)$.
  \end{nlist}
\end{nprop}
...
\begin{proof}\hfill\\
  ...
  \implication{ii)}{iii)} Sean $\varepsilon > 0$, $\esol{x_0}$ solución estable de \ref{lin:PVI},
  y $\delta$ el dado por la estabilidad de $\esol{x_0}$ para $\varepsilon$.
    
  Sea $\psi$ solución de \ref{lin:PVIH} con $\| \psi(t_0) \| < \delta$. Entonces, $y := \psi + \esol{x_0}$ es solución de \ref{lin:PVI} y
  $\| y(t_0) - \esol{x_0}(t_0) \| = \| \psi(t_0) \| < \delta $, y por la estabilidad, $\| \psi(t) \| < \varepsilon$ para cada $t \in [t_0, +\infty)$.
    
  \implication{iii)}{iv)} Sea $\esol{x_0}$ una solución cualquiera de \ref{lin:PVIH}, y sea $\lambda \in \R^+$ tal que $\| \lambda x_0 \| < \delta$, con $\delta$
  ...
  Sea $\delta > 0$ tal que $\delta M < \varepsilon$, y $\esol{x_0}, \barsol{x_0}$ soluciones de \ref{lin:PVI} tales que $\| x_0 - \bar{x_0} \| < \delta$.
