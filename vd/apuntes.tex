...
\begin{nprop}[Propiedades de las aplicaciones diferenciables] \label{prop:propiedades-dif}
  ...
  \item \label{item:carta-difeo}
    si $(V,\phi)$ es una carta de $M$, $\phi: V\to \phi(V)$ es un difeomorfismo.
  \end{nlist}
\end{nprop}
\begin{proof}
...
\begin{nprop}
  Para cada $p\in M$, $v\in \T_{f(p)}N$, $\d f_{p}(v)$ es una derivación.
\end{nprop}
\begin{proof}
  \begin{align*}
...
Para proporcionar una definición de campo de vectores \emph{diferenciable}, que
...
Ya podemos construir la estructura diferenciable en $\T M$:
...
En la demostración de esta proposición daremos la identificación anunciada.
\begin{nprop}
...
Ahora, consideramos $M^{n}\subseteq \R^{N}$ una subvariedad cerrada. Recordamos
...
\subsubsection{Campos diferenciables en el proyectivo}
...
Algunos resultados que permiten extender campos diferenciables:
\begin{nprop}
...
Una observación que será instrumental en lo sucesivo:
\begin{nprop}
