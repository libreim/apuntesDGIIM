...
## Multiprocesador :
Varios threads en paralelo en varios procesador
...
## Core multithread:
Modifican su estructura ILP(Instruction level paralellism) para ejecutar threads en paralelo.
...
b. Multihilo simultáneo(SMT): Se ejecutan en un core superescalar varios cores en paralelo, emitiendo instrucciones de varios threads en un ciclo.
...
La memoria se actualiza de forma *inmediata*(write-through) cada vez que un procesador escribe en su caché lo hace también en memoria principal, lo cual no es muy rentable, o medianet *posescritura*(write-back), si se escribe todo el bloque cuando se desaloja de la caché.

Para intentar hacer la caché más coherente, se puede intentar hacer una *escritura con actualización* (write-update), en la que si un procesador escribe en una dirección de caché, se escribe en las copias de esa dirección en las otras cachés, o una *escritura con invalidación*, en la que antes de modificar una dirección en su caché se invalidan las copias del bloque de la dirección en otras cachés.

### Requisitos del sistema para evitar problemas de incoherencia
...
2. Serializar las escrituras en dirección, viéndose en el mismo orden por todos los procesadores con los componentes conectados por un bus. Si la red no es un bus, el orden en el que las peticiones de escritura llegan al nodo que tiene en MP la dirección sirve para serializar en sistemas de comunicación que garantizan el orden de las transferencias.
