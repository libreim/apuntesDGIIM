...
\item \hyperref[posvar:car-ii]{[C-II]}: $\Psi_{\vectX} = e^{it^T\mu - \frac{1}{2}t^T \Sigma t}$, y
...
sea $\vectX = (X_1,\dots,X_p)^T\sim N_p(\mu,\Sigma)$, con $\Sigma \ge 0$, es
...
\[
\begin{cases} \vectY_1 \sim N_r(H_1\mu,D),\, D> 0 \\ \vectY_2 \equiv H_2\mu \quad c.s. \end{cases}
\]
...
%% TODO: puntualizar en la primera caracterización que la igualdad es en distribución.
Aplicamos a $\vectY_1$ la \hyperref[posvar:c-i]{[C-I]}: tenemos que $\vectY_1$ puede representarse en distribución como
\[
...
\begin{nth}
 ...
 %%  \[
 %% \vectX = \begin{pmatrix} \vectX_1 \\ \vectX_2 \end{pmatrix} \ \ ; \ \ \mu = \mu_1 \\ \mu_2 \end{pmatrix} \ \ ; \ \ \Sigma = \Sigma_{11} & \Sigma_{12} \\ \Sigma_{21} & \Sigma_{22} \end{pmatrix}
 %%  \]
