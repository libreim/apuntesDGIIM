...
Por conveniencia, expresamos la suma de forma cuadrática en el exponente de la forma siguiente:
\[
...
En principio A no tiene por qué ser definida positiva, pero tenemos el teorema de Dijkstra que dice lo siguiente:

\begin{nth}[Teorema de Dijkstra]
...
\qquad A = (a_{ij)) = \begin{pmatrix}A_{(11)} & A_{(12)} \\ A_{(21)} & A_{(22)} \end{pmatrix}
  \qquad S = (s_{ij)) = \begin{pmatrix}S_{(11)} & S_{(12)} \\ S_{(21)} & S_{(22)} \end{pmatrix}
    ...
    Sean $e_i = (0,\dots_{(i)},1,\dots,0)^T$ y $e_j = (0,\dots_{(j)},\dots,0)^T$. Entones, $\mu_i = e_i^T \mu$ y $\Sigma_{ij} = e_i^T \Sigma e_j$. Por tanto, aplicando el th. de Zehna se verificará que:
    \begin{itemize}
    \item $\cap{\mu_i} ) e_i^T \cap{\mu} = e_i^T \bar{\vectX} = \bar{\vectX_i}$ para $i = 1,\dots,p$.
      \item $\cap{\sigma_{ij}} = e_i^T \cap{\Sigma}e_j = e_i^T \frac{A}{N}e_j = \frac{a_{ij}}{N} = s_{ij}$ para $i,j = 1,\dots,p$
    \end{itemize}
...
\[
\begin{cases}
  \cap{\Sigma_{(11)}} = E_1 \Sigma E_1^T = E_1\frac{A}{N}E_1^T = \frac{A_{11}}{N} = S_{11} \\
  \cap{\Sigma_{(22)}} = E_2 \Sigma E_2^T = E_2\frac{A}{N}E_2^T = \frac{A_{22}}{N} = S_{22} \\

  \cap{\Sigma_{(12)}} = E_1 \Sigma E_2^T = E_1\frac{A}{N}E_2^T = \frac{A_{12}}{N} = S_{12} \\
  \cap{\Sigma_{(21)}} = E_2 \Sigma E_1^T = E_2\frac{A}{N}E_1^T = \frac{A_{21}}{N} = S_{21} \\
  \end{cases}
\]


\begin{ndef}[Coeficiente de correlación lineal de Pearson]
  ...
  \[
\cap{p_{ij}} = \frac{\cap{\sigma_{ij}}{\cap{\sigma_i}\cap{\sigma_j}} = \frac{s_{ij}}{s_i s_j} = r_{ij}
  \]
\end{ndef}

% FIXME : cambiar lo necesario 
Además, si notamos por $algo$ a la fila $i-$ésima de la matriz $algo$, entonces $algo$, con lo cual, por el teorema de Zehna, $algo$, siendo $algo$ y $algo$ las filas $i-$ésimas de $A$ y $S$ respectivamente.

Por tanto, dado el coeficiente de correlación lineal múltiple $algo$, se verificará:
\[
algo
\]
...
\begin{nprop}[Distribución de la matriz de cuasi-covarianzas muestral]
...
\begin{nprop}[Relacionando con la distribución de Hotelling]
...
\begin{ndef}
  ...
  Definimos entonces 
  %el \textit{estadístico \(T^2\) de Hotelling} como la distribución de la variable aleatoria dada por
  \[
