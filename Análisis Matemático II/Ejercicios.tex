...
Tenemos que $\varphi$ es continua, pues la preimagen de un entorno básico (bolas abiertas) es un entorno básico. Para verlo, en lugar de $\varphi$ vamos a tomar su extensión a todo el espacio (la llamaremos $\varphi'$), definiéndola de la misma forma ($f \mapsto f+\tilde f$).

\[
\begin{array}{l}
	\varphi'^{-1}(B_\infty(f, \epsilon)) = \{ x\in \mathcal{C}(A,\R^M) : \varphi'(x) = x + 	\tilde f \in B(f, \epsilon)\} =\\
	\{ x\in \mathcal{C}(A,\R^M) : \|f-\tilde f-x\| < \epsilon\} = B(f-\tilde f,\epsilon)
\end{array}
\]
