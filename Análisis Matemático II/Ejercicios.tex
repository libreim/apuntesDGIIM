...
\begin{ejer}
  Tomamos $\Omega = \mathbb R, \mu =$ medida de Lebesgue en $\mathbb R,
  \mu(\mathbb R) = \infty$.

  $f_n(\omega) = \chi_{[n,n+1]}(\omega) \rightarrow 0 \forall \omega \in \R$.

  Porque $\forall \omega \in \R \exists \omega_0 \in \N : \omega < n_0
  \rightarrow f_n(\omega) = 0 \forall n \geq n_0$

  Si $F \subset \R$ medible tal que $\mu(R \setminus F) < \varepsilon <
  \infty$ Luego si $\mu(\R \setminus F) < \infty$ y $\mu(\R) = 0$ entonces
  $\mu(F)=\infty$.
  Con lo que $f_n(\omega)$ no converge uniformemente a 0 en $F$.
