...
\begin{nprop} La composición de funciones medibles es medible. Es decir, si\\ \mbox{$f:(X,\mathcal A) \longrightarrow (Y,\mathcal B)$}, $g:(Y,\mathcal B) \longrightarrow (Z,\mathcal C)$ son medibles, entonces $g\circ f$ es medible.
	\begin{proof}
	Evidente, pues $(g\circ f)^{-1}(C) = f^{-1}(g^{-1}(C)) \in \mathcal A$, por ser $f$ y $g$ medibles.
\end{proof}
\end{nprop}
...
\begin{nprop}[Caracterización de funciones medibles positivas] \label{criterio_medibles} Sea $(\Omega,\mathcal{A})$ un espacio de medida y $f:\Omega \to [0,\infty]$. Entonces: 
...
\begin{proof}\hfill
  ...
  \begin{center}
	\includegraphics[height=15\baselineskip]{fig_th_Lebesgue.pdf}
\end{center}
  Así, tenemos que:
  \[
...
\begin{ndef}[Medida]
  ...
  \begin{nlist}
  \item $\mu(\emptyset) = 0$
  \item \textit{(Aditividad).} $\forall \{A_k\} \subseteq \mathcal A$ con $A_k\cap A_j = \emptyset,\ k \neq j,$ se tiene que:
    $$\mu \left( \bigcup_{k=1}^\infty A_k \right) = \sum_{k=1}^\infty \mu(A_k) $$
  \end{nlist}
  La terna $(\Omega, \mathcal A, \mu)$ se llama \textbf{espacio de medida}.
\end{ndef}

\begin{nprop}[Propiedades de una medida] \label{prop_medida} Si $(\Omega, \mathcal A, \mu)$ es un espacio de medida, se verifican las siguientes propiedades:
  \begin{nlist}
  ...
  \item $\forall \{A_k\} \subseteq \mathcal{A}$ tal que $A_1\supseteq A_2\supseteq \cdots \supseteq A_k \supseteq \cdots,$ si $\mu(A_1) < \infty$ se tiene:
...
Sea $(\Omega,\ \mathcal A=2^\Omega)$ un espacio medible. Para $\omega\in\Omega$, la medida de Dirac  o masa puntual (point mass) es la medida $\delta_\omega:\mathcal A \longrightarrow [0,\infty ]$ dada por 
 \[
...
\begin{ejemplo}[Medida de Lebesgue]
	 Si $(\Omega = \mathbb R^N, \mathcal A= \mathcal B(\mathbb R^N))$ espacio medible, la medida de Lebesgue es $\lambda: \mathcal A \to [0,\infty]$ dada por  
 \[
 \lambda ([a_1,b_1]\times  [a_2,b_2]\times \dots [a_N,b_N]) = (b_1-a_1) (b_2-a_2) \dots (b_N-a_N)
 \]
\end{ejemplo}

Estamos ya en disposición de definir lo que entenderemos por integral de una función medible positiva, en el marco de un espacio de medida $(\W, \mathcal A, \mu)$. Como ya dijimos, el problema se reducirá a estudiar integrales asociadas a funciones simples, las cuales definimos a continuación.
...
\begin{lema} \label{phi}
	Sea $s:\W \to [0,\infty)$ simple. Entonces, $\varphi:\mathcal A \to [0,\infty]$ dada por $$\varphi(E) = \int_E s\, d\mu, \quad \forall E \in \mathcal A$$ es una medida.
...
\begin{nth}[Teorema de la convergencia monótona] \label{tcm}
  ...
  \begin{proof} Empezamos observando que $\displaystyle f(\omega):= \lim _{n\to \infty} f_n(\omega)$ es medible por la \textit{Proposición \ref{p1}}, teniendo en cuenta que $\{f_n(\omega)\}$ es monótona creciente, y por tanto convergente. Veamos ahora las dos desigualdades.
    ...
    \]
    %%% TODO: Probar que los E_n son medibles
    Tenemos que los conjuntos $E_n$ son medibles, y claramente $E_n \subseteq E_{n+1} \ \forall n \in \mathbb{N}$, pues $\fn$ es creciente. Además, se verifica que $$E = \bigcup_{n=1}^\infty E_n$$ Para verlo, sea $\w \in E$. Si $f(\w) = 0 \implies s(\w) = 0 \implies \w \in E_1$. En otro caso, $\rho s(\w) < f(\w)$, pues $\rho < 1$. Entonces, $\exists n \in \mathbb{N}: \w \in E_n$. 
...
Sean \\ $f,g:\Omega \to [0,\infty]$ medibles positivas, $\alpha \in \R, E \in \mathcal{A}$. Entonces, se verifican las siguientes propiedades.
...
\item Relaciones entre la integral y la medida de algunos conjuntos:
  $$\int_\Omega f\, d\mu < \infty \implies \mu(\{\omega \in \Omega: f(\omega) =  \infty\}) = 0$$
  $$\int_\Omega f\, d\mu = 0 \iff \mu(\{\omega \in \Omega : f(w) > 0\}) = 0$$
\end{nlist}
...
\begin{nlist}
    ...
    Veamos ahora la segunda equivalencia:
...
\boxed{\Leftarrow} $\dots$
\end{nlist}
\end{proof}
%%% TODO: Completar demostración.
\end{nprop}

\begin{nprop} \label {phi_2} Sea $f:\W \to [0,\infty]$ medible. Entonces, $\varphi:\mathcal{A} \to [0,\infty]$ dada por $$\varphi(E) = \int_E f\, d\mu, \quad \forall E \in \mathcal A$$
...
\[
  ...
  \]
    ...
    %%% WARNING: ¿Manera adecuada de referenciar un teorema por un nombre cualquiera?
    \[
...
\begin{lema} [Lema de Fatou]
  Si $(\Omega,\mathcal{A},\mu)$ es un espacio de medida, $E\in \mathcal{A}$ y tenemos funciones $f_n:\Omega \to [0,\infty]$ medibles, entonces:
  \[
...
Vamos a ver ahora el Teorema de Fubini y Torelli, que nos permite integrar en un
conjunto $A \subset \R^N$, que se reduce a calcular $N$ integrales en
subconjuntos de R.

Vamos a ver antes una pequeña definición que nos va a ayudar a entender lo que
viene después.
...
\begin{enumerate}
\item Para a.e. $y \in \R^M$ la función $\underset{\big{ f^y: \R^N \rightarrow \R}}{f^y(x) := f(x,y)}$ es integrable en
  $\R^N$. Entonces $\displaystyle \varphi(y) := \int_{\R^N}f^y dm_N$. Esto quiere decir que $m_N
  ( \overbrace{\{ y \in
  \R^M : f^y \text{ no es integrable en } R^N \}}^{A}) = 0 \Leftrightarrow \forall y
  \in R^m \ A$ % FIXME Aquí falta un poco. Era texto que estaba en color y dijo
  % que iba a borrar.

  Vemos que $\varphi$ está definida $\forall y \in \R^M \ A$. Esta función
...
\item Para a.e. $x \in R^N$ la función $f_x(y) := f(x,y)$ es integrable. Esto
  quiere decir que $\varphi (x) := \displaystyle \int_{\R^M} f_x \ dm_M$ es
  integrable en $R^N$, lo que implica que $\exists \displaystyle \int_{\R^N}\psi
  \ dm_N$.
\item $\int_{R^M} \varphi \ dm_M = \int_{\R^M} \psi \ dm_N = \int_{R^N \times R^M} f
  \ dm_{M \times N}$.
\end{enumerate}
\end{nth}
...
\begin{ejemplo}
  ...
  ocurre cuando fijamos un punto $y$. Dado $y \in \R$, $$\underset{\big{x \mapsto f^y(x)
...
% FIXME Demostrar.
\begin{nprop}
  La medida de un intervalo es la longitud del intervalo.
\end{nprop}
