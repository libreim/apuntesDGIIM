...
\begin{ndef}[Funciones medibles positivas]
	Una función $f: \Omega \to X = [0,\infty ]$ se llama función medible positiva.
\end{ndef}
...
Una característica importante de las funciones simples es que siempre se pueden escribir como una suma finita de funciones. Antes de verlo, necesitamos definir la función característica de un conjunto $A$, que denotaremos $\upchi_A$ y viene dada por:
	\[
	\upchi_A(\omega)= \begin{cases}
	1 & si \ \omega \in A\\
	0 & si \ \omega \notin A
\end{cases}
	\]
...
\begin{nprop}[Descomposición canónica]
	Si $s:\Omega \to [0,\infty)$ es simple con \mbox{$s(\Omega) =  \{\alpha_1,\cdots, \alpha_m\}$}, entonces los conjuntos medibles
	\[
	A_k:= \{w \in \Omega: s(\omega)= \alpha_k\} = s^{-1}([0,\alpha_k])\backslash s^{-1}([0,\alpha_k))
	\] 
	son una partición de $\Omega$, y además:
	\[
	s = \sum_{k=1}^m \alpha_k \upchi_{A_k}
	\]
...
\begin{nth}[Convergencia dominada de Lebesgue]
	Sean $f_n : \Omega \rightarrow \R$ medibles y $E \in \mathcal{A}$. Además, suponemos que ${f_n(\omega)} \rightarrow f(\omega)$ c.p. con $\omega \in E$. Finalmente, suponemos que $\exists g: \Omega \rightarrow \R$ integrable tal que $|f_n(\omega)| \leq g(\omega) \forall \omega \in E$. Entonces,
	
	$$ \lim_{n \to \infty} \int_E f_n d \mu = \int_E (\lim_{n \to \infty} f_n ) d \mu$$ 
\end{nth}

\begin{nota}
	Como para toda función $|f_n| \leq g$ y $g$ es integrable, $|f_n|$ también es integrable, luego cada $f_n$ es integrable.
\end{nota}

\begin{nota}
	Como $f(\omega) = \lim_{n \to \infty} f_n(\omega)$ con $f_n$ funciones medibles $\Rightarrow f$ es medible en $\Omega$.
\end{nota}

\begin{proof}
	Definimos $g_n(\omega) := 2g(\omega)\upchi_E(\omega) - \abs{f_n(\omega) - f(\omega)} \chi_E (\omega)$, que es integrable. Es claro que $\{g_n(\omega)\} \rightarrow 2g(\omega)\upchi_E(\omega)$, ya que $f_n(\omega) \rightarrow f(\omega)$ y $\upchi_E$ es acotada.
	
	Vamos a ver además que $g_n \geq 0$ y medible en $\Omega$ porque $|f_n - f| \leq |f_n| + |f| \leq g + g = 2g\upchi_E(\omega)$.
	
	Por el \textit{Lema de Fatou}, 
	$$\liminf_{n \to \infty} \int_{\Omega} g_n d \mu \geq \int_{\Omega} \liminf_{n \to \infty} g_n d \mu = 2 \int_{\Omega} g\upchi_E(\omega)d\mu$$
	
	$$\liminf_{n \to \infty} \left[\int_{\Omega} 2g(\omega)\upchi_E(\omega) d\mu - \int_{\Omega} \abs{f_n(\omega) - f(\omega)}\upchi_E(\omega) d \mu \right] \leq 2\int_{\Omega} g\upchi_E(\omega) d \mu$$
	
	Como recordatorio, si $\exists \lim_{n \to \infty} x_n$ entonces $\liminf_{n \to \infty} (x_n + y_n) = \lim_{n \to \infty} x_n + \liminf_{n \to \infty} y_n$. Además, $\liminf_{n \to \infty} x_n = - \limsup_{n \to \infty} (- x_n)$.
	
	Luego entonces
	
	$$2 \int_{\Omega} g(\omega)\upchi_E(\omega) d \mu + \liminf_{n \to \infty} \left[-\int_{\Omega} \abs{f_n - f} \upchi_E(\omega) d \mu \right]$$
	
	$$ = 2 \int_{\Omega} g \upchi_E(\omega) d \mu - \limsup_{n \to \infty} \int_{\Omega} \abs{f_n - f} \upchi_E(\omega) d \mu$$
	
	En resumen,
	
	$$2 \int_{\Omega} g \upchi_E(\omega) d \mu - \limsup_{n \to \infty} \int_{\Omega} \abs{f_n - f} \upchi_E(\omega) d \mu \geq 2 \int_{\Omega} g \upchi_{E} d \mu$$
	
	$$\int_{\Omega} g \upchi_{E} d \mu = \int_{\Omega} g  d \mu < \infty \Rightarrow \limsup_{n \ to \infty} \int_{\Omega} \abs{f_n - f} \upchi_E d \mu \leq 0$$
	
	$$\Rightarrow 0 \leq \liminf_{n \to \infty} \int_{\Omega} \abs{f_n - f} \upchi_E d \mu \leq \limsup_{n \to \infty} \int_{\Omega} \abs{f_n - f} \upchi_E d \mu = 0$$
	
	$$\Rightarrow \exists \lim_{n \to \infty} \int_{\Omega} \abs{f_n - f} \upchi_E d \mu = 0$$
	...
	$$ \Rightarrow \exists \lim_{n \to \infty} \int_E f_n d \mu = \int_E f d \mu$$
\end{proof}
