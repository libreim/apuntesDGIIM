...
La demostración se puede consultar en el fichero \textit{Ejercicios}.
\end{nth}
...
Si definimos $\varphi_k:= \sigma_k \circ \dots \sigma_2 \circ\sigma_1$, podemos visualizar cada una de las sucesiones anteriores, cada una en una fila:
\[
 \begin{array}{lllllll}
...
\]
As\'{\i}, como la sucesión $\{ f_{\sigma_k \circ \dots \sigma_2 \circ\sigma_1 (n)} (r_k) \} \to f(r_k)$, tenemos que $\{f_{\sigma (n)} (r_k) \}_{n\in\mathbb N} \longrightarrow f(r_k)$ $\forall k\in\mathbb N$, puesto que el comportamiento de un número finito de términos no afecta a la convergencia. Por último, como $\{r_k : k \in \mathbb{N} \}$ era una enumeración de $A \cap \mathbb Q^N$, concluimos que:
\[
