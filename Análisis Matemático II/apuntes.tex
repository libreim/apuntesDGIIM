...
\end{cases}
    ...
    %%% FALTA COMPLETAR CON DIAPOSITIVAS ---------------------------------------
\end{proof}
...
\begin{proof}
	Por el teorema de Lebesgue sabemos que si existe una sucesion $\{s_n\}$ y otra $\{t\}$ simple positivas que convergen respectivamente en f y g. Por la propiedad 3.6 además sabemos que podemos generar una sucesión $\{s_n+t_n\} \rightarrow f+g$ de sucesiones simples positivas. Por último, al aplicar otra vez el teorema de Lesbesgue, nos queda que la función $f+g$ es medible.
	
	La demostración para $ \alpha f$ y para  $fg$ son análogas.
\end{proof}
...
\begin{ndef}[Medida]
	...
	\begin{nlist}
	 	\item $\mu(\emptyset) = 0$
	 	\item $\forall \{A_k\} \subset \mathcal A$ con $A_k\bigcap A_j = \emptyset, \forall k \neq j,$ se tiene
	 	$$\mu \left( \bigcup_{k=1}^\infty A_k \right) = \sum_{k=1}^\infty \mu(A_k) $$
	\end{nlist}
	$(\Omega, \mathcal A, \mu)$ se llama espacio de medida.
\end{ndef}

\textbf{Propiedades de la medida:}
\begin{nlist}
	...
	\]
	\item $\forall \{A_k\} \subset \mathcal{A}$ con $A_1 \subset A_2 \subset ... \subset A_k \subset ...$ se tiene:
	$$\mu \left( \bigcup_{k=1}^\infty A_k \right) = \lim_{k \to \infty}\mu(A_k)$$
	\item $\forall \{A_k\} \subset \mathcal{A}$ con $A_1 \supset A_2 \supset ... \supset A_k \supset ...$ se tiene:
