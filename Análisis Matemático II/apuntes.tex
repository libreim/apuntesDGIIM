...
\begin{nprop}[Propiedades integral función simple] \label{int_simples} Sea $s$ una función simple. Entonces, se verifica lo siguiente:
\vspace{0.5em}
    \begin{nlist}
...
\begin{nth}[Teorema de la convergencia monótona]
  ...
  \begin{proof} Empezamos observando que $\displaystyle f(\omega):= \lim _{n\to \infty} f_n(\omega)$ es medible por la \textit{Proposición \ref{p1}}, teniendo en cuenta que $\{f_n(\omega)\}$ es monótona creciente, y por tanto convergente. Veamos ahora las dos desigualdades.
    ...
    \]
    \boxed{\leq} Para probar esta desigualdad, usaremos el \textit{Lema \ref{phi}}. Ahora, como
    \[
