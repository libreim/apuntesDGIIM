...
\[
	f_n : A \to \R^M\ \ \forall n \in \mathbb{N}
\]

será una sucesión de funciones, y la notaremos $\{f_n\}$. Normalmente estas funciones serán continuas.

\begin{ndef}[Convergencia punto a punto.]
	\[
		\{f_n\}\to f \text{ punto a punto } \iff \ \forall x\in A 
	\]
\end{ndef}


\begin{ndef}[Convergencia uniforme.]
	\[
	\fn \to f \text{ uniformemente} \iff \forall \epsilon>0\ \exists K>0\ n\ge K \implies \abs{f_n(x)-f(x)} < \epsilon\ \forall x \in A
	\]
\end{ndef}
...
Observamos que la convergencia puntual no preserva la continuidad de las funciones. Esto implicaría que, con esta definición de convergencia, el espacio de funciones continuas en un conjunto no sería completo. Además, podemos comprobar que $\fn$ no converge uniformemente a $f$.
...
Vemos que converge uniformemente a cero. Lo importante para esta demostración, y lo que lo será en la mayoría de los casos de convergencia uniforme, es que podemos encontrar un $\epsilon_n$ tal que
\[
