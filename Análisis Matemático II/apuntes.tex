...
\begin{proof}
  ...
  \[ \exists K>0:\ n\ge K \implies \abs{f_n(y)-f(y)}<\dfrac{\epsilon}{3}\ \forall y\in A \]
    Es obvio entonces que si tomamos un punto $a \in A$, $\abs{f_n(a)-f(a)}<\dfrac{\epsilon}{3}$. Dado que $f_n$ es una función continua para cada $n \in \N$, por la definición de continuidad de una función podemos afirmar que:
...
\begin{nth} \label{14}
  ...
  %\begin{proof} \hfill
  %\begin{nlist}
  %   \item Veamos que $S(x)$ es continua en todo el disco de convergencia. Para ello, sea $x \in D(a,R)$, y tomo $R' < R$ tal que $x \in D(a,R')$. Entonces, la serie $\sum_{n\geq 0} a_n (x-a)^n$ converge uniformemente a $S(x)$ en $D(a,R')$, y concluimos que $S(x)$ es continua, por ser el límite uniforme de funciones continuas. 
