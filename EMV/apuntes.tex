...
\begin{ndef}[Probabilidad inducida]
    ...
    .\] para todo \(B \in \mathbb{B}^p\). Es decir, $P_X = P \circ \restr{\boldsymbol X^{-1}}{\mathscr{B}}$.
\end{ndef}
...
%#TODO: ¿Está amsmath incluido? Usar bmatrix de amsmath
  En particular, si $\boldsymbol X= \left( X_{(1)} | X_{(2)}\right)^T :=
  \left(\begin{array}{c | c}
    X_{(1)} & 0 \\ \hline
    0 & X_{(2)}
        \end{array}\right)$, con $X_{(1)}$ de dimensión $(k\times1)$ y $X_{(2)}$ de dimensión $(p-k) \times 1$, se tendría:
  \[
X_{(1)} = \left( I_{k\times k} | 0_{k \times(p-k)} \right)^TX + 0_{k\times 1}
\]
...
\begin{nth}
  ...
  \]
  con $g = (g_1,\dots,g_p): S \mapstp \mathcal T$ tal que $x\mapsto y=g(x)$ biyectiva, por lo que $\exists g^{-1}$ transformación inversa $x = g^{-1}(y)$, que la denotaremos como $g^{-1} = h = (h_1,\dots, h_p)^T : \mathcal T \to S$ con $y \mapsto x =h(y) = g^{-1}(y)$.\\
