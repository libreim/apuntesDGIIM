% Apuntes originales por Santiago de Diego, Braulio Valdivielso y Francisco Luque
...
familia de subconjuntos $\mathscr{A}$ es $\sigma$-álgebra sobre $\Omega$ si: 
\begin{enumerate}
\item $\Omega \in \mathscr{A}$
\item Si $A \in \mathscr{A} \Rightarrow \overline{A} \in \mathscr{A}$
\item Si $\left\{A_n\right\}_{n \in \N} \in \mathscr{A} \Rightarrow \displaystyle \bigcup_{n \in \mathbb{N}} A_n \in \mathscr{A}$
\end{enumerate}
...
\begin{itemize}
	...
	\item $\mathscr{A}$ es cerrada para la  intersección numerable (se obtiene tomando complementarios en la unión numerable).
\end{itemize}

\subsubsection*{Espacio medible}

Al par ($\Omega$, $\mathscr{A}$) se le llama espacio medible, y a los conjuntos que pertenecen a $\mathscr{A}$ se les denomina conjuntos medibles.

\subsection{Límites en sucesiones de conjuntos}

\subsubsection{Límites en sucesiones monótonas}

Si tomamos la relación de inclusión entre conjuntos ($\subseteq$) como una relación de orden, podemos hablar de sucesiones monótonas. En este sentido, una sucesión   $\left\{A_n\right\}_{n \in \mathbb{N}}$  creciente de conjuntos cumpliría que $A_i \subseteq A_{i+1}, \forall i \in \mathbb{N}$, y una sucesión decreciente de conjuntos cumpliría la condición análoga.\\

El límite de una sucesión creciente de conjuntos $\left\{A_n\right\}_{n \in \mathbb{N} }$ se puede definir
$$\lim_{n\to\infty} A_n = \bigcup_{k=1}^{\infty} A_k$$

Esta definición aprovecha la relación de orden entre los conjuntos de la sucesión para afirmar que \textit{si un elemento está en algún conjunto de la sucesión, entonces está en todos los posteriores}, por ello se puede utilizar una unión numerable (que está perfectamente definida) para formalizar el concepto.\\

Con una intuición análoga se define el límite de una sucesión decreciente de conjuntos. Si $\left\{A_n\right\}_{n\in\mathbb{N}}$ es una sucesión decreciente de conjuntos, entonces:
...
Dada una sucesión de conjuntos $\left\{A_n\}\right_{n\in\mathbb{N} }$ podemos considerar el conjunto

$$ B_n = \bigcap_{k=n}^{\infty} A_k $$

y este conjunto contiene aquellos elementos que están en \textbf{todos} los $A_k$ para $k \geq n$. Es fácil probar que la sucesión $\left\{B_n\right\}_{n\in\mathbb{N}}$ es una sucesión creciente de conjuntos, y por tanto se puede obtener el límite $\lim_{n\to\infty} B_n$. Informalmente, ese límite es un conjunto que contiene a todos los elementos de $A_n$ que están en todos los conjuntos $A_k$ a partir de cierto $n\in\mathbb{N}$. Definimos el límite inferior de $A_n$ como

$$ \liminf A_n = \lim_{n\to\infty} B_n = \bigcup_{n\in\mathbb{N}} \bigcap_{k=n}^{\infty} A_n $$

Análogamente, podríamos definir la sucesión de conjuntos $\left\{C_n\right\}_{n\in\mathbb{N}}$ como

$$ C_n = \bigcup_{k=n}^{\infty} A_n $$

Cada $C_n$ contiene todos los elementos que están presentes en algún $A_k$ para $k \geq n$. Es fácil también ver que la sucesión $\left\{C_n\right\}_{n\in\mathbb{N}}$ es una sucesión decreciente de conjuntos, y por tanto su límite también está bien definido. Se puede definir entonces el límite superior de $\left\{A_n\}\right_{n\in\mathbb{N}}$ como
...
Una vez definidos los conceptos sobre conjuntos con los que vamos a trabajar, pasamos a definir las funciones sobre conjuntos. Si ($\Omega$, $\mathscr{A}$) y($\Omega'$, $\mathscr{A}'$) son espacios medibles, llamaremos función de conjunto a una función:
\[ 
\begin{array}{llll}
...
A raíz de esta definición podemos definir también lo que se conoce como función inversa. Dada una función $X$, la función inversa de $X$, $X^{-1}$, asigna a cada conjunto $A' \in \mathscr{A}'$ el conjunto $A \in \mathscr{A}$ tal que $X(A) = A'$. Esto sujeto a las condiciones usuales de inyectividad y sobreyectividad. La propiedad básica que cumplen las funciones inversas es que preservan las operaciones e inclusiones de conjuntos.

\subsection{Concepto de $\sigma$-aditividad}
\begin{ndef}[Función aditiva, $\sigma$-aditiva y subaditiva]
  Sea un conjunto $\Omega$ y una $\sigma$-álgebra $\mathscr{A}$ sobre $\Omega$ y sea
  $$\varphi : \mathscr{A} \rightarrow \mathbb{R}$$
  %% \begin{center}
  %% $\varphi (A)$ es único\\
  %% \end{center}
  \begin{itemize}
    \item Se dice que $\varphi$ es \textbf{subaditiva} si $\varphi(A \cup B) \leq \varphi(A) + \varphi(B)$
  \item  Se dice que $\varphi$ es \textbf{aditiva} si $\varphi\left(\displaystyle\sum_{k=1}^{n}A_k\right)=\displaystyle\sum_{k=1}^n\varphi(A_k)$
  \item  Se dice que $\varphi$ es \textbf{$\sigma$-aditiva} si $\varphi\left(\displaystyle\sum_{k=1}^\infty A_k\right)=\displaystyle\sum_{k=1}^\infty \varphi(A_k)$
  \end{itemize}
...
\subsection{Continuidad en funciones sobre conjuntos}
...
Sea $\varphi$ una función $\sigma$-aditiva sobre una $\sigma$-álgebra $\mathscr{A}$. Entonces, existen $C,D \in \mathscr{A}$ tales que $\varphi (C) = \sup \varphi$ y $\varphi (D) = \inf \varphi$
\end{nth}

\begin{proof}
Probaremos la existencia del conjunto $C$. La del conjunto $D$ es análoga. Si $\varphi(A) = \infty$ para algún $A \in \mathscr{A}$, entonces podemos establecer $A = C$ y la demostración del teorema es trivial. Entonces, supongamos que $\varphi < \infty$ y dado que el valor $-\infty$ está excluido, $\varphi$ es finita.\\

Entonces, existe una sucesión $\left\{A_n\}\right \subset \mathscr{A}$ tal que $\varphi(A_n) \to \sup \varphi$. Sea $A = \cup A_n $ y para cada $n$, consideramos la partición de $A$ en $2^n$ conjuntos $A_{nm}$ de la forma $\displaystyle \cap_{k=1}^n A'_k$, donde $A'_k = A_k$ o $A - A_k$. Para $n < n'$, cada conjunto $A_{nm}$ es una suma finita de conjuntos $A_{n'm'}$. Sea ahora $B_n$ la suma de los conjuntos $A_{nm}$ para los cuales $\varphi$ es no negativa. Si no hay ninguno, entonces $B_n = \emptyset$. Entonces, tenemos por un lado que $A_n$ es la suma de algunos de los $A_{nm}$, y por otro lado, para $n' > n$, cada conjunto $A_{n'm'}$ está dentro de $B_n$ o son disjuntos. Entonces tenemos
...
A la tupla ($\Omega$, $\mathscr{A}$, $\mu_{\mathscr{A}}$) se le denomina espacio de medida.\\
...
Una vez definido el concepto de medida, vamos a dar ahora el de probabilidad. Una probabilidad $\mathcal{P}$ sobre un espacio medible ($\Omega$, $\mathscr{A}$) es una medida que además cumple que $\mathcal{P}(\Omega)=1$. Por tanto, tiene las siguientes propiedades:\\

$\mathcal{P}(\emptyset)=0$\\
$\forall A \in \mathscr{A}, 0 \leq \mathcal{P}(A) \leq 1$\\
Es una función $\sigma$-aditiva\\

De forma análoga al concepto de espacio de medida, podemos definir ahora el de espacio probabilístico. Un espacio probabilístico es una tupla formada por un conjunto $\Omega$, una $\sigma$-álgebra sobre ese conjunto, $\mathscr{A}$, y una función de probabilidad $\mathcal{P}$.
...
\begin{nth}
Sea $\mu^\circ$ una medida exterior sobre la $\sigma$-álgebra $\mathscr{A}$, y sea $\mathscr{A}^\circ$ la clase de conjuntos $\mu^\circ$-medibles:
\begin{itemize}
\item $\mathscr{A}^\circ$ es un $\sigma$-campo
\item $\mu^\circ$ en $\mathscr{A}^\circ$ es una medida
\end{itemize}
\end{nth}

\begin{proof}
Primero, veamos que $\mathscr{A}^\circ$ es un álgebra. Sea $A \in \mathscr{A}^\circ$, entonces $A^c \in \mathscr{A}^\circ$ dado que la definición de $\mu^\circ$-medibilidad es simétrica. Sean entonces $A,B \in \mathscr{A}^\circ$. Entonces:
...
Dado entonces que $\mathscr{A}^\circ$ es cerrada bajo la operación complementario e intersección finita, lo es para la unión finita, así que nos encontramos ante un álgebra. Veamos ahora que $\mu^\circ$ es aditiva en $\mathscr{A}^\circ$. Sean $A,B \in \mathscr{A}^\circ$ disjuntos. Entonces:
$$\mu^\circ(A+B) = \mu^\circ((A+B)A) + \mu^\circ((A+B)A^c) = \mu^\circ(A) + \mu^\circ(B)$$
Y teniendo que $\mu^\circ(A) \geq \mu^\circ(\emptyset) = 0$, $\mu^\circ$ sobre $\mathscr{A}^\circ$ es una función aditiva y positiva. Para completar la prueba, veamos que $\mu^\circ$ es $\sigma$-aditiva. Sean $A_n \in \mathscr{A}^\circ$, $A = \displaystyle \sum A_n$. Tomamos los conjuntos $B_n = \displaystyle \sum_{k=1}^n A_k \in \mathscr{A}^\circ$. Tenemos que:
...
Además queda demostrado que $A \in \mathscr{A}^\circ$, así que tenemos que $\mathscr{A}^\circ$ es un $\sigma$-campo.
...
Dada una medida $\mu$ sobre un álgebra $\mathcal{C}$, existe una extensión $\mu^\circ$ sobre la $\sigma$-álgebra minimal sobre $\mathcal{C}$ ($\mathscr{A}(\mathcal{C})$). Además, si $\mu$ es finita, la extensión es única.
\end{nth}
...
$$ \stackrel{\varepsilon \to 0}{\Rightarrow} \mu^\circ(D) \geq \mu^\circ(AD) + \mu^\circ(A^cD), A \in \mathscr{A}^\circ \Rightarrow \mathcal{C} \subseteq \mathscr{A}^\circ, \mathscr{A}(\mathcal{C}) \subseteq \mathscr{A}^\circ $$
Por los dos teoremas que hemos demostrado antes, la restricción $\mu^\circ$ sobre $\mathscr{A}(\mathcal{C})$ es una extensión de $\mu$ a una medida sobre $\mathscr{A}(\mathcal{C})$. Ahora, veamos que es única. Para ello, sean $\mu_1, \mu_2$ extensiones de $\mu$. Sea entonces el conjunto $\mathcal{M} = \{A: \mu_1(A) = \mu_2(A)\} \Rightarrow \mathcal{C} \subset \mathcal{M}$. Tenemos entonces que:
...
Sean ahora $\left\{A_n\}\right \subset \mathcal{M}$ monótona, entonces $\mu_1(\lim A_n) = \lim \mu_1(A_n) = \lim \mu_2(A_n) = \mu_2(\lim A_n) \Rightarrow \mathcal{M}$ cerrada frente al límite de sucesiones monótonas $\Rightarrow \mathcal{M} \sigma$-álgebra que contiene a $\mathcal{C}$, por tanto, contiene a la $\sigma$-álgebra minimal sobre $\mathcal{C}$
$$ \mathscr{A}(\mathcal{C}) \subseteq \mathcal{M} \Rightarrow \mu_1 = \mu_2 \: en \: \mathscr{A}(\mathcal{C})$$
...
Una vez definidos los conceptos sobre espacios de medida y espacios probabilísticos, vamos a aproximarnos al concepto de función medible. Para ello, daremos dos definiciones de función medible, para demostrar más adelante que estas dos definiciones son equivalentes. Empecemos con la definición constructiva de función medible. Dado que nos interesa que el codominio sea $\mathbb{R}$, trabajaremos con funciones definidas entre un espacio medible ($\Omega$, $\mathscr{A}$) y ($\mathbb{R}$, $\mathcal{B}$), donde $\mathcal{B}$ representa la $\sigma$-álgebra de Borel.\\
...
\textbf{Definición descriptiva de función medible:} Sea una función $\varphi : \mathscr{A} \rightarrow \mathcal{B}$. Se dice que $\varphi$ es medible si $\forall B \in \mathcal{B} \Rightarrow \varphi^{-1}(B) \in \mathscr{A}$. Es decir, una función se dice medible si la imagen inversa de todo conjunto medible es medible.\\
...
\begin{nth} Teorema de medibilidad\\
...
\begin{proof}
$$(gX)^{-1}(B)=X^{-1}(g^{-1}(B))\in \mathscr{A}$$
\qed
\end{proof}

\subsection{Convergencia en probabilidad y convergencia casi segura}

Vamos a establecer ahora criterios que nos permitan comparar variables aleatorias. Dado un espacio de probabilidad ($\Omega$, $\mathscr{A}$, $\mathcal{P}$), se dice que dos variables aleatorias son equivalentes si:
...
\subsubsection{Definición de convergencia uniforme en u}
...
Dado un espacio probabilístico $(\Omega, \mathscr{A}, P)$ y una v.a. $X$, se define la esperanza matemática de $X$ como:
...
\subsubsection{Teorema de la convergencia dominada}
\begin{nth}
...
Dado un espacio probabilístico ($\Omega$, $\mathscr{A}$, $\mathcal{P}$) y una variable aleatoria $X$, se define el momento r-ésimo o de orden r de la variable $X$ a:
...
\item ($\Omega$, $\mathscr{A}$, $\mathcal{P}$) $\stackrel{\mathcal{P}}{\rightarrow}$ ($\mathbb{R}$, $\mathcal{B}$), definimos ahora $\mathcal{P}^X(\mathcal{B}) = \mathcal{P}(X^{-1}(\mathcal{B}))$. $\mathcal{P}^X$ es una probabilidad, ya que $\mathcal{P}^X(\mathbb{R}) = 1, \mathcal{P}^X(\mathcal{B}) \geq 0, \mathcal{P}^X(\sum B_j) = \sum \mathcal{P}^X(B_j)$, y $EX = \displaystyle \int_{x \in \mathbb{R}} x d \mathcal{P}^X(x)$; $Eg(X) = \displaystyle \int_{y \in \mathbb{R}} g d \mathcal{P}^Y(y) = \displaystyle \int_{x \in \mathbb{R}} g(x) d \mathcal{P}^X(x)$
\end{enumerate}
...
\subsubsection{Distribución inducida}
...
\\\\
Sea $(\Omega,\mathscr{A},P) \, , \, A_n \in \mathscr{A}$. Entonces:
\begin{itemize}
<<<<<<< HEAD
...
\subsubsection{Variables aleatorias independientes y espacios producto}
...
\subsubsection{Ley de Bernouilli de los grandes números} % Pag 244 del Loeve
...
\subsubsection{Ley fuerte de Borel de los grandes números} % Pag 244 del Loeve
...
\begin{proof}*
\end{proof}
\subsubsection{Centramiento y truncamiento}
...
\section{El problema central del límite}
...
\subsection{Primer teorema del límite central (Problema clásico)}
...
\begin{nth} \textbf{Segunda extensión del problema clásico central del límite}\\
