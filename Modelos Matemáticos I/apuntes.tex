...
\begin{ndef}
    Dada una matriz $A \in \mathcal M_k (\mathbb C)$ con espectro
    $$ \sigma(A) = \{ \lambda_1, \lambda_2, \hdots, \lambda_r \}$$
    donde $r \leq k$.
    Decimos que $\lambda_1$ es el \textit{valor propio dominante} si 
    \begin{itemize}
        \item $|\lambda_1| > |\lambda_2| \geq \hdots \geq |\lambda_r|$
        \item Es simple
    \end{itemize}
\end{ndef}
...
\begin{ejemplo}
    \begin{align*}
        ...
        A &= \begin{pmatrix}
            ...
            0 & 0 & 0 \\
            0 & 0 & 1
        \end{pmatrix}, \sigma(A) = \{0, 1 \} \ \ \ \ \lambda_1 = 1 \text{ dominante} \\
        B &= \begin{pmatrix}
            1 & 0 \\
            0 & -1
        \end{pmatrix}, \sigma(B) = \{ 1, -1 \} \ \ \ \ |\lambda_1| = |\lambda_2|, \text{ no hay valor propio dominante}
    \end{align*}
\end{ejemplo}

\begin{nth}
    Si $A \in \mathcal M_k (\mathbb C)$ es una matric con valor propio dominante $\lambda_1$, entonces la sucesión $$ \left\{ \frac{1}{\lambda_1^n}A^n \right\}_{n \geq 0}$$ converge a una matriz $Q \in \mathcal M_k (\mathbb C)$ que verifica:
    \begin{itemize}
...
\begin{proof}
    ...
    $$J = \begin{pmatrix}
        \lambda_1 & & & & \\
                  ...
                  & & & \ddots & \\
                  & & & & J_r
    \end{pmatrix} \ \ |\lambda_1| > |\lambda_i|, i = 1, 2, \hdots, r \ \ \rho(J_i) < |\lambda_1|$$
    %FIXME Es posible que falte algo aquí.
    ...
    \begin{align*}
        \label{}
        \frac{1}{\lambda_1^n} A^n = \frac{1}{J_1^n}(PJ^nP^{-1}) = P \frac{1}{\lambda_1^n}J^nP^{-1} = P
        %FIXME Faltan cosas aquí.
    \end{align*}
    ...
    \begin{itemize}
        \item Sea $Ker(A - \lambda_1I) =$ subespacio propio asociado al valor propio $\lambda_1$ de dimensión 1 = $\langle v_1 \rangle$ = $\langle p_1 \rangle$ .
            ...
            Por otro lado, $Q = P\theta P^{-1} \Rightarrow QP = PQ \Rightarrow Q \begin{pmatrix}
                p_1 & p_2 & \hdots & p_k
            \end{pmatrix} = \begin{pmatrix}
                p_1 & p_2 & \hdots & p_k
            \end{pmatrix} \begin{pmatrix}
...
\item
    $$ \begin{rcases}
        ...
        \vdots \\
        Qp_k = 0
    \end{rcases} \text{ luego } Im Q = \langle p_1 \rangle = Ker (A - \lambda_1 I)$$
\item
    \begin{align*}
        \label{}
        Q^2 = P\theta P^{-1} = P \begin{pmatrix}
            1 & 0 & \hdots & 0 \\
            0 & 0 & \hdots & 0 \\
            \vdots & \vdots & \ddots & \vdots \\
            0 & 0 & \hdots & 0
...
\item 
    ...
    \begin{align*}
        \label{}
        QA = (P\theta P^{-1})A = \left[ P \left( \lim_{n \to \infty} \frac{1}{\lambda^n_1}J^n \right) P^{-1} \right] A
    \end{align*}
...
\end{proof}
    ...
    % TODO No está copiado porque dice antcc que lo va a copiar
