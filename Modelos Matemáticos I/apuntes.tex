...
Sin embargo, no todas las normas matriciales son inducidas. Un ejemplo es la \emph{norma de Frobenius}, que no es una norma inducida: $$\|A\|_F = \sqrt{\sum_{i=1}^k \sum_{j=1}^k |a_{ij}|^2}$$
...
\end{equation}
   Como sabemos que $v\ne0$, que $\displaystyle \lim_{n\to\infty} \la^n = 0 \iff |\la| < 1$ y que $\eqref{eq:teorema}$ se verifica para cualquier $\la$ valor propio de $A$, concluimos que necesariamente $\rho(A) < 1$.
    ...
    Supongamos que $r$ es el número de valores propios de $A$ distintos entre sí. Entonces, tenemos que: $$J = \begin{pmatrix}
        J_1 & & & \\
            & J_2 & & \\
            & & \ddots & \\
            & & & J_r
    ...
    %%% FIXME: La forma de J_i está mal, faltan los '1' encima de la diagonal. Revisar demostración.
