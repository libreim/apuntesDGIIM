...
Para algún índice $i \in \{1,2,\hdots,N-1\},\  a_ia_{i+1}> 0$.
     ...
     \begin{proof}
         ...
         Suponiéndolo cierto hasta $N-1$, por hipótesis de inducción, veámoslo ahora para $N$. \begin{align*}
            ...
            % FIXME: No sé cómo poner esto de forma que lo que está tras el igual a continuación no se salga de la página y no mueva las matrices superiores hacia la derecha
...
\section{Matrices positivas, estrictamente positivas y estocásticas}
...
Sea $A=(a_{ij})\in M_{d\times d}(\R) $ una matriz. 
	...
	Una matris $A$ se dice débilmente positiva o no negativa  $A\geq 0$ si $a_{ij} \geq 0 \ \ \ \forall i,j$
\end{ndef}
