...
\begin{nth}[Teorema del punto fijo de Banach o teorema de la contracción]
	Supongamos que $E$ es un espacio métrico completo. Sea $G:E \to E$ que es una contracción, es decir una aplicación Lipschitziana con constante de Lipschit menor que 1:
	\[
	\exists c \in (0,1): \ \ d(G(x),G(y))\leq c d(x,y) \quad \forall x,y \in E
	\]
	Entonces, $\exists! x_o \in E$ tal que $x_o = G(x_o)$, es decir, $x_0$ es un punto fijo de G.
	Además, se cumplen:
	\begin{nlist}
	\item Si tomo $y_1\in E$ y evalúo $G(y_1) = y_2 \in E$. Ahora, calculo $G(y_2) = y_3 \in E$. Así, tomamos sucesivamente $G(y_{n-1}) = y_n \in E$, lo que define una sucesión $\{y_n\}$ convergente tal que $\{y_n\}\to x_0$
	\item \[
	d(y,y_0) \leq \frac{d(y,G(y))}{1-c} \quad \forall y \in E
	\]
\end{nlist}
\end{nth}
\begin{proof}[]
	Se deja propuesto como ejersisio.
	Como indicación, habrá que probar
	\[
	d(y_{n+2},y_{n+1})=d(G(y_{n+1}),G(y_n)) \leq cd(y_{n+1},y_n)
	\]
	Tenemos que ver cuánto vale $d(y_m,y_n)\leq d(y_m,y_{m-1}) + d(y_{m-1},y_{m-2}) + ... + d(y_{n+1},y_n)$\\
	Entonces:
	\[
	d(G(y_{n+1}),G(y_n)) \leq cd(y_{n+1},y_n) \leq ccd(y_n,y_{n-1}) \leq c...cd(y_2,y_1)
	\]
	De esta forma, la sucesión sería de Cauchy
	La sucesión de $y_n$ es también convergente, por ser de Cauchy en un espacio métrico.
	
	Hay que terminar esta demostración a la que se llega a una contradicción a menos que $x=y$
\end{proof}
