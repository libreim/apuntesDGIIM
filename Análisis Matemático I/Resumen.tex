...
\begin{nth}
	Un conjnuto $A\subset M$ es cerado $\iff$ todos los puntos de acumulación de A pertenecen a A.
\end{nth}
\section{Sucesiones}
...
\begin{nprop}
	Sea $v_n \to v \in \mathbb{R}^n \iff$ cada sucesión de coordenadas converge a la coordenada correspondiente de $v$ como una sucesión en $\mathbb{R}$
\end{nprop}

\begin{nprop}
	\begin{itemize}
	\item Un conjunto $A\subset M$ es cerrado $\iff \forall x_n \in A $ con $x_n$ convergente, el límite es un elemento de A.
	
	\item Para un conjunto $B \subset M, \ \ x \in cl(B) \iff \ $ existe una sucesión $x_n\in B$ tal que $x_n \to x$
...
\begin{nprop}
	...
	\item Una sucesión de Cauchy enun espacio métrico debe estar acotada
...
\begin{ndef}[Compacto]
	Un subconjunto A de un espacio métrico M es compacto si todo recubrimiento abierto de A contiene un recubrimiento finito.
\end{ndef}
...
\begin{nth}
	Los conjuntos conexos por arcos son conexos.
\end{nth}
\begin{proof}
Sean x,y dos puntes de A. Suponemos $x \leq y$ (si fuera al revés, cambiamos los nombres).
...
\begin{ndef}[Límite]
	...
	\lim{x\to x_0}f(x) = b
	\]
	Si para todo $\epsilon > 0  \ \exists \delta > 0 $ tal que $\forall x \in A$ que sea distinto de $x_0$ y $d(x_0,x)$, entonces $d'(f(x),b) < \epsilon$ 
\end{ndef}

\begin{ndef}[Función continua]
	Una función $f:A \to B$ es continua en un punto $x_0$ de su dominio $\iff \forall \epsilon > 0 \ \ \exists > 0 : \ \forall x \in A$ que cumpla que $d(x,x_0)< \delta \implies d'(f(x),f(x_0)) < \epsilon$
\end{ndef}
...
\end{nth}
\begin{proof}
	Sea $\{x_n\}\subseteq K$ cualquiera. Entonces $\{f(x_n)\} \subseteq f(K)$. Como $K$ es compacto, existe $\sigma: \mathbb{N} \rightarrow \mathbb{N}$ estrictamente creciente tal que $\{x_{\sigma(n)}\} \rightarrow a\in K$. Luego, $\{f(x_{\sigma(n)})\}\rightarrow f(a)\in K$ y queda probado que $f(A)$ es compacto.
\end{proof}
...
\begin{ndef}[Uniformemente continua]
	Sean (M,d) y (N,p) espacioes métricos, $A \subset M$ , $f:A \to N$ y $B \subset A$. Decimos que f es uniformemente continua en el conjunto B si $\forall \epsilon > 0 \ \exists \delta > 0 :  \ x,y \in B \ \ y \ \ d(x,y) < \delta \implies p(f(x),f(y)) < \epsilon$
\end{ndef}
...
\begin{ndef}
	...
	Una transformación $f:A\subset \R^n \to \R^m$ es diferenciable en $x_0 \in A$ si existe una transformación lineal, denotada $Df(x_0): \R^n \to \R^m$ llamada diferencial de $f$ en $x_0$ tal que:
	 ...
	 \]
	 Donde $Df(x_0)(x-x_0)$ es el valor de la aplicación lineal aplicada al vector $(x-x0)$.\\
...
\end{nth}
\begin{ndef}[Gradiente (caso $f:A \to R$)]
	En el caso de que $f:A \to \R$, entonces $Df(x)$ es una matriz $1xn$. El vector cuyas componentes son iguales a las de $Df(x)$ se denomina gradiente de $f$ y se denota $\triangledown f$.
	\[
...
\begin{ndef}[Derivada direccional]
	...
	Sea f una función escalar definida en una vecindad de $x_0\in \R^n$ y sea $e\in \R^n$ un vector unitario, entonces:
	\[
	\frac{d}{dt}f(x_0+te)|_{t= 0} =  \lim_{t\to 0} \frac{f(x_0+te) -f(x_0)}{t}
	\]
	es la derivada direccional de $f$ en $x_0$ en la dirección e.
	
	Se puede afirmar que la derivada direccional en la dirección de e es igual a $Df(x_0)e$.
\end{ndef}
...
\begin{nth}[Teorema del valor medio]
	\begin{nlist}
...
\begin{nprop}
	Si $f:A \to \R^m$ dos veces diferenciable en el conjunto abierto A con $D^2 f$ continua, entonces $D^2f$ es simétrica, es decir:
	\[
...
\begin{ndef}
	Sea $f:A\subset \R^n \to \R$ donde A es abierto. Si existe una vecindad de $x_0 \in A$ en la que $f(x_0)$ es máximo, es decir si $f(x_0) \geq f(x) \ \forall x$ en la vecindad, entonces $x_0$ es un punto de máximo local y $f(x_0)$ es un máximo local de f. Análogamente se define un mínimo local de f. Un punto es extremo si es un máximo o un mínimo local de f. Un punto $x_0$ es un punto crítico si $f$ es diferenciable en ese punto y $Df(x_0) = 0$ siendo $x_0$ ese punto.
\end{ndef}
