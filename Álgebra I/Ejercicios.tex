...
\subsection{\LARGE{Ejercicio 16 - 2 en $\Z [\sqrt{n}]$}}

\textbf{Probar que en el anillo $\Z[\sqrt{2}]$ el entero cuadrático $2+\sqrt{2}$ y su conjugado $2-\sqrt{2}$ son asociados.
...
\subsection{\LARGE{Ejercicio 17 - 3 en $\Z [\sqrt{n}]$}}

\texbf{a) Determinar un entero de Gauss $\alpha \in \Z[i]$, tal que al dividirlo por 3 da resto i, mientras su resto
al dividirlo 3 + 2i es 1 + i. \\
b) Determina enteros de Gauss $ x, y \in \Z[i]$, con $N(x) \leq 18$, tales que 4x + (3 + 3i)y = -1 + 5i}
