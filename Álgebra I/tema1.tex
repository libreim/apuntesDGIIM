...
Sea $K$ un cuerpo, y consideremos $\mathcal{M}_{mxn}(K)$ y  $\mathcal{M}_{n}(K)$. Imaginemos ahora que $A\leq K$ es un subanillo. De esta forma, podemos concebir las matrices que están este en este conjunto: $\mathcal{M}_{mxn}(A)$ ó  $\mathcal{M}_{n}(A)$ .
...
\[
(\sum_{i=1}^m p_{ki}a_{i1}, \cdots , \sum_{i=1}^n p_{ki}a_{im}) = \sum_{i=1}^n p_{ki}(a_{i1},\cdots , a_{im})
\]
