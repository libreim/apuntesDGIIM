...
\begin{proof}
	...
	Por tanto, sea $r$ el primer índice tal que $p$ no divide a $a_r$. Tomemos también $s$ el primer índice tal que $s$ no divide a $b_s$. Ahora, vamos a fijarnos en el coeficiente $c_{r+s}$ de $fg$. Este coeficiente se expresa como:
	 ...
	 De esta expresión, podemos ver que $p/c_{r+s}$, que $\forall i < r$ entonces $p/a_ib_j \implies p/\sum_{i+j =r+s}a_ib_j$ por dividir a los $a_i$. Ocurre lo mismo con el mismo término pues p divide a $b_j$. Si despejáramos $a_rb_s$ veríamos que $p/a_rb_s$ pues divide a todos los factores, pero p es un primo, por tanto si divide a un producto tiene que dividir a alguno de los factores, pero como habíamos dicho que no divide a ninguno de los dos, hemos llegado a una contradicción y por tanto $fg$ es primitivo.
...
\begin{proof}
$(i)$ Una factorización de un polinomio de grado 0 será irreducible si es irreducible en A. U(A[x]) = U(A)\\
$(ii)$ $\displaystyle \boxed{\Rightarrow}\ $ Si f es irreduciblem el contenido tiene que ser o una unidad o un asociado. No puede ser asociado porque...
Como c(f)/f, si f es irreducible $\Rightarrow$ c(f) = 1 $\Rightarrow$ f es primitivo. Vamos a probar que es irreducible en K[x]. Supongamos que f no es irreducible en K[x] $\Rightarrow$ f = $\phi \psi$ con $gr(\phi) \geq 1$ y $gr(\psi) \geq 1$
$$
 \left .
      \begin{matrix} 
  ...
  $$
  Aplicando contenidos sobre esta igualdad nos queda, $c(bdf) = c(acgh) \Rightarrow bdc(f) = acc(gh) \Rightarrow bd = ac \Rightarrow f = gh $ CONTRADICCIÓN f es irreducible, no podemos expresarlo como producto de dos no unidades.
...
\textit{Observación:} Sea $gr(\phi) \geq 1,\ \phi \in$ K[x], $\phi=\frac{a}{b}f$ con $f$ primitivo. $\phi$ es irreducible en K[x] $\Leftrightarrow$ f es irreducible en A[x].
