...
En particular, si $p$ es un irreducible de $\ent$ y $f(x)$ es un irreducible de $\ent_p[x]$ de grado $n \implies \ent_p[x]_{f(x)}$ es un cuerpo con $^np$ elementos. Lo notamos $F_pn = \ent_p[x]_{f(x)}$
...
\end{enumerate}
\begin{proof}
	$\ent_n \cong \ent/(n)\implies \exists x \in Z_n :  ax = b \iff \exists[x] \in \ent/(n) : [a][x] = [b] \iff$
\end{proof}
\section{AQUÍ FALTA CONTENIDO UN POCO UN POCO}


Ahora, si $x = x_0 + kn' \quad k \in $, los $x$ que satisfacen nuestro problema original son los restos de estos elementos:
\[
x_0 + ...........
\]

\section{AQUI FALTA MÁS CONTENIDO UN POQUITO}

En la ecuación $ax = b$ de $\ent_n$, sabemos que tiene solucion si y solo si b divide a d. Supuesto que esto ocurra, si $n' = \frac{n}{d}$ existe una única solución $x_0$ en $0 \leq x_0 \leq n'$ y la ecuación tiene exactamente $d$ soluciones: $x_0,x_0+n',x_0+2n',...,x_0+(d-1)n'$
...
\begin{nprop}
\begin{nlist}
	\item Todo primo es irreducible
