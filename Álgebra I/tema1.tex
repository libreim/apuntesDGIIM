...
$ \forall a \in \mathbb Z $ si $(a,m) = 1 \implies a^{\varphi(m)} = 1 $ mod(m)

\textbf{Corolario.}

Si p es irreducible $ \forall a \in \mathbb Z_{p} $	 \hspace{0.25cm} $ a^{p} = a $ en $ \mathbb Z_{p} $

\textbf{Teorema (pequeño) de Fermat}

$ \forall a \in \mathbb Z $ $ a^{p} = a \cdot mod(p) $ si p es irreducible

\textbf{Demostración del corolario:}

Partiendo de la hipótesis del teorema demostraremos el corolario:

$ m = p $ $ a \neq 0 $ en  $ \mathbb Z_{p} \implies (a,p) = 1 \implies a^{\varphi(p)} = 1 $ en $ \mathbb Z_{p} $
