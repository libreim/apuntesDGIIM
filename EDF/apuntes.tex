...
\begin{ndef}[Solución de una ecuación diferencial]
  Sea $\upphi : D \subseteq \R^{k+2} \to \R$ una ecuación diferencial e $I \subseteq \R$ un intervalo abierto. Diremos que $x:I\to \R$ es una solución de la ecuación si cumple: 
  \begin{itemize}
...
Vamos a ver ahora algunos ejemplos de ecuaciones diferenciales de primer orden y sus soluciones (marcadas con subíndices).
...
\begin{ejemplo}
¿Es $\varphi(t) = \sqrt{2t+1}$ una solución de la ecuación $x(t)x'(t) = 1$?
...
Nótese que al dividir por $x$, hemos eliminado la solución $x\equiv 0$, pero como no era solución son la misma ecuación.
...
\begin{proof}
  ...
  \[
      e^{-kt}\varphi(t) = A, \text{ con} \ A\in \R \implies \varphi(t) = Ae^{kt}, \ \forall t \in I
  \]
\end{proof}
