...
A $\mathcal{W}(f_0,\dots,f_n)$ se le llama \emph{wronskiano} de $f_0,\dots,f_n$. Cabe notar que esta no es condición suficiente.
...
\begin{nprop}
  Sean $I$ un intervalo y $\{\varphi_1,\dots,\varphi_n\}$ soluciones de \eqref{eclinealsup}. Son equivalentes:
  \begin{nlist}
  \item $\{\varphi_1,\dots,\varphi_n\}$ son un sistema fundamental de soluciones, es decir, una base del espacio de soluciones.
...
Para hallar la solución particular $x_p$, usamos el método de variación de las constantes.

Sea $x_p(t) = \sum_{i=1}^n c_i(t) \varphi_i(t)$, con $c_i$ tales que

\[ 
\left\{\begin{array}{>{\displaystyle}l}
  \sum_{i=1}^n c_i'(t) \varphi_i(t) = 0\\
  \vdots\\
  \sum_{i=1}^n c_i'(t) \varphi_i^{n-1}(t) = 0
...
\begin{align*}
  x_p'(t) & = \cancelto{0}{\sum_{i=1}^n c_i'(t) \varphi_i(t)} + \sum_{i=1}^n c_i(t) \varphi_i'(t)\\
  x_p''(t) & = \cancelto{0}{\sum_{i=1}^n c_i'(t) \varphi_i'(t)} + \sum_{i=1}^n c_i(t) \varphi_i''(t)\\
  \vdots\\
  x_p^{n)}(t) & = \cancelto{0}{\sum_{i=1}^n c_i'(t) \varphi_i^{n-1)}(t)} + \sum_{i=1}^n c_i(t) \varphi_i^{n)}(t)\\
\end{align*}


Las condiciones dadas en \eqref{cond}, para cada $t\in I$, forman un sistema compatible determinado, por ser $\mathcal{W}(\varphi_1,\dots,\varphi_n)(t) \ne 0 \ \forall t\in I$.
Por la regla de Cramer:

\[
c_i'(t) = 
\frac{\begin{vmatrix}
  \varphi_1(t) & \dots & \stackrel{i)}{0} & \dots & \varphi_n(t)\\
  \varphi_1' & \dots & 0 & \dots & \varphi_n'(t)\\
  \vdots & & \vdots & & \vdots\\
  \varphi_1^{n-1)}(t) & \dots & b(t) & \dots & \varphi_n^{n-1)}(t)\\
\end{vmatrix}}{\mathcal{W}(\varphi_1,\dots,\varphi_n)(t)} \in \mathscr{C}(I)
\]
...
\begin{ndef}
  Sea $f\in C([a,b], \R ^n )$, con componentes integrables. Definimos
