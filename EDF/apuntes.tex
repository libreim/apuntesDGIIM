...
\left\{\begin{array}{>{\displaystyle}l}
  ...
  \sum_{i=1}^n c_i'(t) \varphi_i'(t) = 0\\
  \vdots\\
  \sum_{i=1}^n c_i'(t) \varphi_i^{n-1}(t) = b(t)
...
\begin{align*}
  ...
  x_p''(t) & = \cancelto{0}{\sum_{i=1}^n c_i'(t) \varphi_i'(t)} + \sum_{i=1}^n c_i(t) \varphi_i''(t)\\
  \vdots\\
  x_p^{n)}(t) & = b(t) + \sum_{i=1}^n c_i(t) \varphi_i^{n)}(t)\\
\end{align*}
...
\begin{align*}
  ...
  b(t) + \sum_{i=1}^k c_i(t) \left[ \sum_{j=1}^k\underbrace{ \left( \varphi_j^{k)} + \sum_{t=1}^{k-1}a_t \varphi_j^{t)} \right)}_{=0 \text{ ($\varphi_j$ solución de \eqref{eclinhom})} }  \right]
\end{align*}
