...
\begin{ndef}[Solución de una ecuación diferencial]
  ...
  \item $x \in \mathscr{C}^k(I)$ tal que $(t, x(t), x^{(1)}(t), \dots, x^{(k)}(t)) \in D$
  \end{itemize}
...
\begin{ndef}[Unicidad global de soluciones]
  Dado un problema de valores iniciales, se dice que tiene una solución globalmente única si existe una
  solución $x$ definida en un intervalo maximal $I$ tal que si $x_1$ es otra solución definida en $I_1$, entonces $I_1 \subseteq I$ y $x_1 = x|_{I_1}$.
\end{ndef}

\begin{ndef}[Unicidad local de soluciones]
  Dado un problema de valores iniciales, con la restricción $x(t_0) = x_0$, se dice que tiene una solución localmente única si dadas dos soluciones $x_1,x_2$ definidas en $I_1,I_2$ respectivamente, $\exists \tilde{I} \subseteq I_1 \cap I_2$, con $t_0 \in \tilde{I}$ tal que $x_1|_{\tilde{I}} = x_2|_{\tilde{I}}$.
\end{ndef}
...
Sean $P, Q: \R^2 \to \R$ funciones homogéneas de índice $\alpha$, y sea la ecuación

\[ x' = \frac{P(t,x)}{Q(t,x)} \]

Entonces

\[ x' = \frac{P(t,tx/t)}{Q(t,tx/t)} =  \frac{t^{\alpha}P(1, \frac{x}{t})}{t^{\alpha}Q(1, \frac{x}{t})} = f\left(\frac{x}{t}\right) \]
...
\begin{nth}
Sean $P,Q \in \mathscr{C}^1(D)$ con $D$ dominio de $\R^2$ y estrellado en $x_0$, con $\dfrac{\partial P}{\partial x} = \dfrac{\partial Q}{\partial t}$ en $D$.
Entonces $\exists F \in \mathscr{C}^2 (D)$ tal que $\dfrac{\partial F}{\partial t} = P$ y $\dfrac{\partial F}{\partial x} = Q$
\end{nth}

\begin{proof}
Suponemos, sin pérdida de generalidad, que $x_0 = (0,0)$.
\[
