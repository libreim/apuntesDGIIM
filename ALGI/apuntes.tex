...
\usepackage{array} % Más opciones para el entorno array
...
\begin{ndef}[Monomio en $r$ variables]
  ...
  es un monomio en $r$ variables con coeficientes en $A$. Si $p = (p_1, \dots, p_r)$, denotamos $X^p = \prod_{i=0}^r x_i^{p_i}$.
\end{ndef}
...
\begin{ndef}[Anillo de polinomios en $r$ variables sobre un anillo conmutativo]
  ...
  \[
      A[x_1, \dots, x_r] = \left\{ \sum_{(b,p) \in B\times P} bX^p : 
      \begin{array}{l}
...
\begin{nprop}
  ...
  \[
     A[x_1, \dots, x_{r-1}][x_r] = A[x_1, \dots, x_r]
  \]
\end{nprop}
